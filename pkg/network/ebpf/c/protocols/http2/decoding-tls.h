...
static __always_inline bool tls_read_hpack_int_with_given_current_char(tls_dispatcher_arguments_t *info, __u64 current_char_as_number, __u64 max_number_for_bits, __u64 *out) {
    ...
    __u64 next_char = 0;
    bpf_probe_read_user(&next_char, sizeof(__u8), info->buf + info->off);
    if ((next_char & 128) == 0) {
        info->off++;
...
// otherwise.
static __always_inline bool tls_read_hpack_int(tls_dispatcher_arguments_t *info, __u64 max_number_for_bits, __u64 *out) {
    __u64 current_char_as_number = 0;
    bpf_probe_read_user(&current_char_as_number, sizeof(__u8), info->buf + info->off);
    info->off++;
...
static __always_inline bool tls_parse_field_literal(tls_dispatcher_arguments_t *info, http2_header_t *headers_to_process, __u64 index, __u64 global_dynamic_counter, __u8 *interesting_headers_counter) {
    __u64 str_len = 0;
    if (!tls_read_hpack_int(info, MAX_6_BITS, &str_len)) {
        return false;
    }
...
static __always_inline __u8 tls_filter_relevant_headers(tls_dispatcher_arguments_t *info, dynamic_table_index_t *dynamic_index, http2_header_t *headers_to_process, __u32 frame_length) {
    __u8 current_ch;
...
#pragma unroll(HTTP2_MAX_HEADERS_COUNT_FOR_FILTERING)
    for (__u8 headers_index = 0; headers_index < HTTP2_MAX_HEADERS_COUNT_FOR_FILTERING; ++headers_index) {
        ...
        bpf_probe_read_user(&current_ch, sizeof(__u8), info->buf + info->off);
        info->off++;
        ...
        if (is_indexed) {
            ...
            if (!tls_parse_field_literal(info, current_header, index, *global_dynamic_counter, &interesting_headers)) {
                break;
            }
        }
    }
...
static __always_inline void tls_process_headers(tls_dispatcher_arguments_t *info, dynamic_table_index_t *dynamic_index, http2_stream_t *current_stream, http2_header_t *headers_to_process, __u8 interesting_headers) {
    http2_header_t *current_header;
...
#pragma unroll(HTTP2_MAX_HEADERS_COUNT_FOR_PROCESSING)
    for (__u8 iteration = 0; iteration < HTTP2_MAX_HEADERS_COUNT_FOR_PROCESSING; ++iteration) {
        ...
        if (current_header->type == kExistingDynamicHeader) {
            ...
            BUF_READ(http2_path, dynamic_value.buffer, info->buf + current_header->new_dynamic_value_offset);
...
static __always_inline void tls_process_headers_frame(tls_dispatcher_arguments_t *info, http2_stream_t *current_stream, dynamic_table_index_t *dynamic_index, struct http2_frame *current_frame_header) {
    const __u32 zero = 0;
    ...
    __u8 interesting_headers = tls_filter_relevant_headers(info, dynamic_index, headers_to_process, current_frame_header->length);
    tls_process_headers(info, dynamic_index, current_stream, headers_to_process, interesting_headers);
}

static __always_inline void tls_parse_frame(tls_dispatcher_arguments_t *info, http2_ctx_t *http2_ctx, struct http2_frame *current_frame) {
    ...
    if (current_frame->type == kHeadersFrame) {
        tls_process_headers_frame(info, current_stream, &http2_ctx->dynamic_index, current_frame);
    }
...
static __always_inline void tls_fix_header_frame(tls_dispatcher_arguments_t *info, char *out, frame_header_remainder_t *frame_state) {
    ...
    case 1:
        bpf_probe_read_user(out + HTTP2_FRAME_HEADER_SIZE - 1, 1, info->buf + info->off);
        break;
    case 2:
        bpf_probe_read_user(out + HTTP2_FRAME_HEADER_SIZE - 2, 2, info->buf + info->off);
        break;
    case 3:
        bpf_probe_read_user(out + HTTP2_FRAME_HEADER_SIZE - 3, 3, info->buf + info->off);
        break;
    case 4:
        bpf_probe_read_user(out + HTTP2_FRAME_HEADER_SIZE - 4, 4, info->buf + info->off);
        break;
    case 5:
        bpf_probe_read_user(out + HTTP2_FRAME_HEADER_SIZE - 5, 5, info->buf + info->off);
        break;
    case 6:
        bpf_probe_read_user(out + HTTP2_FRAME_HEADER_SIZE - 6, 6, info->buf + info->off);
        break;
    case 7:
        bpf_probe_read_user(out + HTTP2_FRAME_HEADER_SIZE - 7, 7, info->buf + info->off);
        break;
    case 8:
        bpf_probe_read_user(out + HTTP2_FRAME_HEADER_SIZE - 8, 8, info->buf + info->off);
        break;
    }
    return;
}
...
#pragma unroll(HTTP2_FRAME_HEADER_SIZE)
        for (__u32 iteration = 0; iteration < HTTP2_FRAME_HEADER_SIZE && new_frame_state.remainder + iteration < HTTP2_FRAME_HEADER_SIZE; ++iteration) {
            bpf_probe_read_user(&new_frame_state.buf[iteration], sizeof(__u8), info_copy.buf + info_copy.off + iteration);
        }
    ...
    if (bpf_map_update_elem(&tls_http2_iterations, info, iteration_value, BPF_NOEXIST) >= 0) {
...
int uprobe__http2_tls_frames_parser(struct pt_regs *ctx) {
    ...
    // to the maximum number of frames we can process. For that we are checking if the iteration context already exists.
    // If not, creating a new one to be used for further processing
    http2_tail_call_state_t *tail_call_state = bpf_map_lookup_elem(&tls_http2_iterations, info);
    ...
    http2_ctx->http2_stream_key.tup = info_copy.tup;
    normalize_tuple(&http2_ctx->http2_stream_key.tup);
    http2_ctx->dynamic_index.tup = info_copy.tup;
...
#pragma unroll(HTTP2_FRAMES_PER_TAIL_CALL)
    for (__u8 index = 0; index < HTTP2_FRAMES_PER_TAIL_CALL; index++) {
        ...
        info_copy.off = current_frame.offset;

        tls_parse_frame(&info_copy, http2_ctx, &current_frame.frame);
    }
...
delete_iteration:
    bpf_map_delete_elem(&http2_iterations, info);
