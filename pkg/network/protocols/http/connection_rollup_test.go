...
func TestGetTranslatedKey(t *testing.T) {
	t.Run("same key", func(t *testing.T) {
		...
		t1 := aggregator.GetTranslatedKey(c1)
		t2 := aggregator.GetTranslatedKey(c1)
	...
	t.Run("same key, flipped order", func(t *testing.T) {
		...
		t2 := aggregator.GetTranslatedKey(c2)
	...
	t.Run("same IPs, different ephemeral ports", func(t *testing.T) {
		...
		t3 := aggregator.GetTranslatedKey(c3)
