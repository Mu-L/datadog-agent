...
func TestGRPCScenarios(t *testing.T) {
	...
	ebpftest.TestBuildModes(t, []ebpftest.BuildMode{ebpftest.Prebuilt, ebpftest.RuntimeCompiled, ebpftest.CORE}, "with TLS", func(t *testing.T) {
		grpcSuite := &USMgRPCSuite{
...
func (s *USMgRPCSuite) TestSimpleGRPCScenarios() {
	...
	tests := []struct {
	...
	}{
		{
			...
			clientsSetupFn: func(t *testing.T, clientsCount int, withTLS bool) {
				clients, cleanup := getClientsArray(t, 1, withTLS)
				defer cleanup()
				client := clients[0]
