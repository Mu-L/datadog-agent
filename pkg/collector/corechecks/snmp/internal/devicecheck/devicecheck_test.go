...
`)
	...
	deviceCk.SetSender(report.NewMetricSender(sender, "", nil, report.NewInterfaceRateMap()))
...
`)
	...
	// with hostname
	deviceCk.SetSender(report.NewMetricSender(sender, "device:123", nil, report.NewInterfaceRateMap()))
	deviceCk.sender.Gauge("snmp.devices_monitored", float64(1), []string{"snmp_device:1.2.3.4"})
