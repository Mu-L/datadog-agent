...
`)
	...
	deviceCk.SetSender(report.NewMetricSender(sender, "", deviceCk.GetDeviceID(), nil, report.NewInterfaceRateMap()))
...
`)
	...
	// with hostname
	deviceCk.SetSender(report.NewMetricSender(sender, "device:123", deviceCk.GetDeviceID(), nil, report.NewInterfaceRateMap()))
	deviceCk.sender.Gauge("snmp.devices_monitored", float64(1), []string{"snmp_device:1.2.3.4"})
